...
int main( void )
{
    ...
    GLuint programID = LoadShaders("base.vert", "base.frag");

    // grab uniforms to modify
    GLuint u_MVP = glGetUniformLocation(programID, "MVP");
    GLuint u_sun_position = glGetUniformLocation(programID, "sun_position");
    GLuint u_sun_color = glGetUniformLocation(programID, "sun_color");
    GLuint u_texture = glGetUniformLocation(programID, "tex");
    ...
    // Load assets
    std::string asset_paths[] = {"cube.obj", "teapot.obj", "capsule.obj"};
    ModelAsset assets[3];
    int i = 0;

    for (auto path : asset_paths) {
        load_asset(assets[i], path);
        assets[i].programID = programID;
        assets[i].drawMode = GL_TRIANGLES;
        assets[i].drawStart = 0;
        assets[i].drawType = GL_UNSIGNED_SHORT;
        i++;
    }
    ...
    do {
        ...
        if(cameraState == TRACKBALL){
            ...
            if (!io.WantCaptureMouse && mouseLeftState == GLFW_RELEASE && oldMouseLeftState == GLFW_PRESS && (glfwGetTime() - mouseLeftPressTime) < 0.2) {
                glm::vec3 out_origin;
        ...
        for (auto entity : entities) {
            ...
            glUniform3fv(u_sun_position, 1, &sun_position[0]);
            glUniform3fv(u_sun_color, 1, &sun_color[0]);
    
            glActiveTexture(GL_TEXTURE0);
            glBindTexture(GL_TEXTURE_2D, asset->texture);
            // Set our uniform sampler to use Texture Unit 0
        ...
        }
        {
            ...
            if (ImGui::CollapsingHeader("Add Entity", ImGuiTreeNodeFlags_DefaultOpen)){
                ...
                if (ImGui::BeginCombo("##asset-combo", asset_paths[asset_current].c_str()))              {
                    for (int n = 0; n < IM_ARRAYSIZE(asset_paths); n++)
                    {
                        bool is_selected = (asset_current == n); // You can store your selection however you want, outside or inside your objects
                        if (ImGui::Selectable(asset_paths[n].c_str(), is_selected))
                            asset_current = n;
                        if (is_selected)
    ...
    glDeleteVertexArrays(1, &assets[0].vao);
